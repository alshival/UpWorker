...
public partial class WebViewViewModel : ObservableRecipient, INavigationAware
{
    ...
    public void OnNavigatedTo(object parameter)
    {
        ...
        // Update the source based on the passed URL parameter if available
        if (parameter is Uri uri)
        {
            Source = uri; // Directly set the Uri if the parameter is already a Uri
        }
        else if (parameter is string urlString && Uri.TryCreate(urlString, UriKind.Absolute, out Uri parsedUri))
        {
            Source = parsedUri; // Parse the string to Uri and set it
        }
